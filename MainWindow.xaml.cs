        ...
        private void SeekBar_MouseMove(object sender, System.Windows.Input.MouseEventArgs e)
{
    ...
    var previewTime = TimeSpan.FromMilliseconds(ratio * _mediaPlayer.Length);
    ThumbnailTimeLabel.Text = FormatTime(previewTime);

    // サムネイル取得
    GenerateThumbnail(ratio);
...
private void GenerateThumbnail(double ratio)
{
    if (_mediaPlayer == null || _libVLC == null || _mediaPlayer.Media == null) return;

    try
    {
        var mediaPath = _mediaPlayer.Media.Mrl;
        ...
        System.Threading.Tasks.Task.Run(() =>
        {
            try
            {
                ...
                using var thumbPlayer = new MediaPlayer(_libVLC);
                thumbPlayer.Media = media;
                thumbPlayer.Mute = true;

                var tcs = new System.Threading.Tasks.TaskCompletionSource<bool>();

                thumbPlayer.Playing += (s, e) =>
                {
                    thumbPlayer.Time = timeMs;
                    System.Threading.Thread.Sleep(200);

                    var tempFile = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "animeplayer_thumb.png");
                    thumbPlayer.TakeSnapshot(0, tempFile, 160, 90);
